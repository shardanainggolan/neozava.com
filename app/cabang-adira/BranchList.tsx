  ...
  return (
    <div className="relative bg-white rounded-2xl border border-gray-100 overflow-hidden">
      ...
      <div className="relative z-10 px-4! py-4!">
        {/* Province chip */}
        <span className="inline-block text-[10px] font-bold uppercase tracking-wider text-[#9a0000] bg-[#fdf0f0] px-2! py-0.5! rounded-full mb-2!">
          {province}
        </span>
        ...
        <div className="flex gap-2! items-start mb-3!">
          <svg className="w-4! h-4! shrink-0 mt-0.5 text-[#9a0000]" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            ...
            <div className="flex flex-col gap-0.5">
              {telList.map((t, i) => (
                <a
                  key={i}
                  href={`tel:${t}`}
                  className="text-[12px] font-medium text-[#9a0000]"
                >
                  {formatPhone(t)}
                </a>
              ))}
        ...
        <Link
          href={`/cabang-adira/${branch.slug}`}
          className="w-full flex items-center justify-center gap-2! py-2.5! rounded-xl bg-[#9a0000] text-white text-[12px] font-bold active:scale-95 transition-transform"
        >
      ...
      <div className="px-5! pt-5! pb-3! bg-white border-b border-gray-100">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9! pr-4! py-2.5! rounded-xl bg-gray-50 border border-gray-200 text-[13px] text-gray-900 placeholder-gray-400 outline-none focus:border-[#9a0000] focus:bg-white transition-colors"
          />
        </div>
      </div>
